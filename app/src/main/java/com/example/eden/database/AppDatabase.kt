...
//                            it.edenDao().deleteAllCommunities()
    ...
    private suspend fun addData(it: AppDatabase){
        if(it.edenDao().getCommunityCount()==0){
            ...
            //5
            it.edenDao().upsertPost(Post(0, "What I never understood: yes Kojima was forced to quit the project but why not starting a new one with new name? Based on PT", containsImage = true, isCustomImage = false, imageUri = R.drawable.kojima_pt.toString(),
            ...
            it.edenDao().upsertComment(
                Comment(0, text = "My 2nd is a TV", postId = 13, communityId = 6))

            it.edenDao().upsertComment(
                Comment(0, text = "Mount fuji has truly inspired a million photographs",
                    postId = 3, communityId = 1))
            ...
            it.edenDao().upsertComment(
                Comment(0, text = "This is the coolest picture of a cub I have ever seen.", postId = 1, communityId = 1))
